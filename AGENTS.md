...
- 本项目不使用 Vercel；前端 review 以本地运行截图、录屏或按需配置的非 Vercel 共享预览为准。
...
1. **Bootstrapping**: 任务开始时强制读取 `SOUL.md`, `WORKING_CONTEXT.md`, `PROJECT_STATUS.md` 并检查当前 Git 分支。确认处于正确的新分支。
2. **Execution**: 执行代码修改与任务研发。
3. **Persistence**: 任务结束前，必须更新 `WORKING_CONTEXT.md`（包括顶部的 YAML frontmatter），并在最后回复中明文声明："【记忆更新检查】：已确认 WORKING_CONTEXT.md 已更新"。
...
- risks
- frontend reference design and implementation screenshot/recording/local preview notes，若有 UI
...
- 前端设计协作采用 Figma + Build web app/Codex + 本地可验收截图/录屏：Figma 定视觉事实，Build web app/Codex 迭代实现，截图/录屏/本地预览说明提供验收证据。
- 前端实现前应使用 Figma plugin/skill 读取目标 frame；实现后用 `figma-design-review`、实现截图/录屏/本地预览说明对照做视觉验收。
