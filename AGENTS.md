...
## Project Rules & Red Lines

- v1 是内部测试版，不开放公共注册。
- 唯一角色：`student`、`admin`。
- 核心模块：AI Solver、Problem Bank、Article Plaza、Personal Library。
- admin 能力嵌入相关模块，v1 不做大型后台。
- **严谨性与防幻觉**：凡涉及业务强依赖的数据（如标准解答、关联推荐等），必须基于系统内真实且明确的前置数据（如数据库检索结果），严禁利用 AI 编造（Hallucinate）数据兜底。当数据不足时，UI 必须如实处理降级或展示明确的空状态。
- **安全第一**：严禁泄露 API Key、Secrets。严禁将 `OPENROUTER_API_KEY` 暴露给客户端或日志。
- **权限隔离**：严禁绕过 RLS 或后端 Auth 检查。Student 绝对不能拥有 mutated public records 的权限。
