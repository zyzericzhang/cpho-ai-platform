...
- Vercel deployment and preview URLs for frontend review。
...
- 前端设计协作采用 Figma + Build web app + Vercel preview：Figma 定视觉事实，Build web app/Codex 迭代实现，Vercel preview 提供可验收链接。
- 前端实现前应使用 Figma plugin/skill 读取目标 frame；实现后用 `figma-design-review`、实现截图和 Vercel preview 对照做视觉验收。
